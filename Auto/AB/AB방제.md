...
> AB자동방제는 자동방제를 하고자 하는 면적에 상응하는 가상의 직사각형을 상정하고 그것의 한쪽 모서리의 양쪽 끝을 각각 A, B 지점으로 지정한 후, 두 지점을 기반으로 직사각형 모양의 방제경로를 생성하여 사용합니다.

1. AB방제 시작하기  
    아래의 붉은 테두리로 표시된 버튼을 클릭하면 그 아래로 방제경로 생성 방향을 선택하기 위한 두가지의 버튼이 보입니다. 생성방향은 A, B 점을 이은 라인을 기준으로 왼쪽 또는 오른쪽으로 직사각형의 방제경로를 생성하기 위해 필요합니다. 원하는 방향의 버튼을 클릭하십시오. 같은 버튼을 다시 누르면 아래의 버튼들이 사라집니다.
    <br>
...
1. B 지점 설정  
    * A 버튼을 클릭한 후 가상의 사각형의 나머지 모서리의 위치에 기체를 이동시킨 후 하단의 B 버튼을 클릭합니다. B 버튼이 클릭되면 이미 선택한 A 지점과의 라인을 기준으로 시작시 선택한 방향으로 계산된 가상의 사각형의 방제 경로가 생성되어 보이게 됩니다.
    <br>
...
1. 생성된 경로 회전 기능 - 경로 수정  
   TODO

1. 생성된 방제경로를 사용하려면 기체에 경로 데이터를 전송해야 합니다. 하단에 표시된 'UPLOAD' 버튼을 클릭하여 전송합니다. 원활한 데이터 전송을 위해서는 일정 성능(스펙) 이상의 안드로이드 단말기를 사용해야 합니다.  
    <img width="400" src="./Images/ab_4.png"><br>

1. 방제경로가 정상적으로 전달되면 경로가 정상적으로 전달되었다는 알림과 함꼐 하단에 'PLAY' 버튼이 보이게 됩니다. 이 버튼을 누르면 자동방제를 시작하게 됩니다.  
